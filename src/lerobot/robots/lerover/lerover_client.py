...
from .config_lerover import LeRoverClientConfig

logger = logging.getLogger(__name__)


class LeRoverClient(Robot):
    """Remote client for controlling LeRover robot with autonomy features"""

    config_class = LeRoverClientConfig
    name = "lerover_client"

    def __init__(self, config: LeRoverClientConfig):
        import zmq
    ...
    def connect(self) -> None:
        """Establishes ZMQ sockets with the remote LeRover robot"""

        if self._is_connected:
            raise DeviceAlreadyConnectedError(
                "LeRoverClient is already connected. Do not run `robot.connect()` twice."
            )
        ...
        if self.zmq_observation_socket not in socks or socks[self.zmq_observation_socket] != zmq.POLLIN:
            raise DeviceNotConnectedError("Timeout waiting for LeRover Host to connect expired.")

        self._is_connected = True
        logger.info(f"Connected to LeRover at {self.remote_ip}")
    ...
    def get_observation(self) -> dict[str, Any]:
        """
        Capture observations from the remote LeRover robot: current follower arm positions,
        present wheel speeds (converted to body-frame velocities: x, y, theta),
        camera frames, and autonomy status.
        """
        if not self._is_connected:
            raise DeviceNotConnectedError("LeRoverClient is not connected. You need to run `robot.connect()`.")
    ...
    def _from_keyboard_to_base_action(self, pressed_keys: np.ndarray):
        """Convert keyboard input to base action, including autonomy controls"""
        # Handle autonomy controls
        if self.teleop_keys["toggle_autonomy"] in pressed_keys:
            self.autonomy_enabled = not self.autonomy_enabled
            logger.info(f"Autonomy {'enabled' if self.autonomy_enabled else 'disabled'}")

        if self.teleop_keys["emergency_stop"] in pressed_keys:
            self.autonomy_enabled = False
        ...
        return {
            ...
            "y.vel": y_cmd,
            "theta.vel": theta_cmd,
            "autonomy_active": float(self.autonomy_enabled),
            "emergency_stop": 0.0,  # Will be set if emergency key pressed
        }
    ...
    def send_action(self, action: dict[str, Any]) -> dict[str, Any]:
        """Command LeRover to move. Includes autonomy control commands."""
        if not self._is_connected:
            raise DeviceNotConnectedError(
                "LeRoverClient is not connected. You need to run `robot.connect()`."
            )

        # Send the action over ZMQ
        self.zmq_cmd_socket.send_string(json.dumps(action))

        # TODO: Remove the np conversion when it is possible to record a non-numpy array value
    ...
    def disconnect(self):
        ...
        if not self._is_connected:
            raise DeviceNotConnectedError(
                "LeRover is not connected. You need to run `robot.connect()` before disconnecting."
            )
    ...
    def toggle_autonomy(self, enabled: bool = None):
        """Toggle or set autonomy mode"""
        if enabled is not None:
            self.autonomy_enabled = enabled
        else:
            self.autonomy_enabled = not self.autonomy_enabled
        logger.info(f"LeRover autonomy {'enabled' if self.autonomy_enabled else 'disabled'}")
    ...
    def emergency_stop(self):
        ...
        logger.warning("Emergency stop sent to LeRover")